...
import java.net.URL;
import java.util.ArrayList; //lol
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		this.terrainDeJeu.setOnMouseClicked(ev -> {
            ...
            if(this.poseTour) {//on vérifie qu'on a cliqué sur le bouton de création de tour
                ...
                System.out.println("x: " + t.getX()/32 + " y: "+ t.getY()/32);
	...
	private void initAnimation() {
		...
		KeyFrame kf = new KeyFrame(
				...
				(ev ->{
					...
					for(int i=0; i<this.env.getActeurs().size();i++) {
						this.env.getActeurs().get(i).deplacer();
						System.out.println(this.env.getActeurs().get(i).getId());
						PersonnageVue persovue = (PersonnageVue) terrainDeJeu.lookup("#"+env.getActeurs().get(i).getId());
						System.out.println(persovue);
						persovue.seDeplacer();
					}
				})
				);
	...
	void addSoldat(ActionEvent event) {
		for(int i =0; i<5; i++) {
			...
			//	this.terrainDeJeu.getChildren().add(persovue);
				terrainDeJeu.getChildren().add(new PersonnageVue(soldat1));
			}
			...
			//	this.terrainDeJeu.getChildren().add(persovue);
				terrainDeJeu.getChildren().add(new PersonnageVue(monstre2));
			}
			...
			//	this.terrainDeJeu.getChildren().add(persovue);
				terrainDeJeu.getChildren().add(new PersonnageVue(geant3));
			}
			else {
